    ...
    public static partial class HostWriter
    {
            ...
            DotNetSearchOptions dotNetSearchOptions = null)
        {
            ...
            try
            {
                ...
                {
                    // chmod +755
                    File.SetUnixFileMode(appHostDestinationFilePath, UnixFileMode.UserRead | UnixFileMode.UserWrite | UnixFileMode.UserExecute |
...
internal static class FileExtensions
{
    extension(File)
    {
        public static void SetUnixFileMode(string path, UnixFileMode mode)
        {
            ...
            } while (res == -1 && Marshal.GetLastWin32Error() == EINTR);

            if (res == -1)
                throw new Win32Exception(Marshal.GetLastWin32Error(), $"Could not set file permission {Convert.ToString(octal, 8)} for {path}.");

            [DllImport("libc", SetLastError = true)]
             static extern int chmod([MarshalAs(UnmanagedType.LPStr)] string pathname, int mode);
        }
    }
}
#endif
