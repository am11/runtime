        ...
        public static RuntimeConfig FromFile(string path)
        {
            ...
            if (File.Exists(path))
            {
                var reader = new Utf8JsonReader(File.ReadAllBytes(path));
                {
                    ...
                    JsonObject frameworks = runtimeOptions["frameworks"] as JsonObject;
                    if (frameworks != null)
                    {
                        foreach (var framework in frameworks)
                        {
                            runtimeConfig.WithFramework(Framework.FromJson((JsonObject)framework.Value));
                        }
                    }

                    JsonObject includedFrameworks = runtimeOptions["includedFrameworks"] as JsonObject;
                    if (includedFrameworks != null)
                    {
                        foreach (var includedFramework in includedFrameworks)
                        {
                            runtimeConfig.WithFramework(Framework.FromJson((JsonObject)includedFramework.Value));
                        }
                    }
