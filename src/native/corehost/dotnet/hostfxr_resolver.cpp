...
hostfxr_resolver_t::hostfxr_resolver_t(const pal::string_t& app_root)
{
    ...
    else if (!pal::is_path_fully_qualified(m_fxr_path))
    {
        ...
        trace::error(PAL_X("Path to %s must be fully qualified: [%s]"), LIBFXR_NAME, m_fxr_path.c_str());
        m_status_code = StatusCode::CoreHostLibMissingFailure;
    }
    ...
    {
        trace::error(PAL_X("The library %s was found, but loading it from %s failed"), LIBFXR_NAME, m_fxr_path.c_str());
