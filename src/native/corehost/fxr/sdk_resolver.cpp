...
pal::string_t sdk_resolver::resolve(const pal::string_t& dotnet_root, bool print_errors, pal::string_t* out_resolved_root) const
{
    if (trace::is_enabled())
    {
        ...
        trace::verbose(
            PAL_X("Resolving SDKs with version = '%s', rollForward = '%s', allowPrerelease = %s"),
            requested.empty() ? PAL_X("latest") : requested.c_str(),
            to_policy_name(roll_forward),
            allow_prerelease ? PAL_X("true") : PAL_X("false"));
        if (has_custom_paths)
        {
            trace::verbose(PAL_X("  paths = ["));
            for (const pal::string_t& path : paths)
            {
                trace::verbose(PAL_X("    %s"), path.c_str());
            }
            trace::verbose(PAL_X("  ]"));
        }
    }
    ...
    for (auto&& dir : locations)
    {
        ...
        {
            trace::verbose(PAL_X("SDK path [%s] does not exist"), dir.c_str());
            continue;
        }
    ...
    {
        trace::verbose(PAL_X("SDK path resolved to [%s]"), resolved_sdk_path.c_str());
        return resolved_sdk_path;
    }

    if (print_errors)
        print_resolution_error(dotnet_root, PAL_X(""));
...
void sdk_resolver::print_resolution_error(const pal::string_t& dotnet_root, const pal::char_t *main_error_prefix) const
{
    if (!error_message.empty())
    {
        ...
        trace::error(PAL_X("%s%s"), main_error_prefix, error_message.c_str());
        return;
    }

    bool sdk_exists = false;
    const pal::char_t *no_sdk_message = PAL_X("No .NET SDKs were found.");
    if (!requested_version.is_empty())
    {
        pal::string_t requested = requested_version.as_str();
        trace::error(
            PAL_X("%sA compatible .NET SDK was not found.\n")
            PAL_X("\n")
            PAL_X("Requested SDK version: %s"),
            main_error_prefix,
            requested.c_str());
        ...
        if (has_global_file)
        {
            trace::error(PAL_X("global.json file: %s"), global_json.path.c_str());
            if (has_custom_paths)
            {
                trace::error(PAL_X("  Search paths:"));
                for (const pal::string_t& path : paths)
                {
                    trace::error(PAL_X("    %s"), path.c_str());
                }
            }
        }

        trace::error(PAL_X("\nInstalled SDKs:"));
        sdk_exists = sdk_info::print_all_sdks(dotnet_root, PAL_X(""));
        if (!sdk_exists)
        ...
        if (has_global_file)
        {
            trace::error(PAL_X("Install the [%s] .NET SDK or update [%s] to match an installed SDK."), requested.c_str(), global_json.path.c_str());
        }
        else
        {
            trace::error(PAL_X("Install the [%s] .NET SDK or create a global.json file matching an installed SDK."), requested.c_str());
        }
    }
    else
    {
        trace::error(PAL_X("%s%s"), main_error_prefix, no_sdk_message);
        if (has_custom_paths && paths.empty())
        {
            trace::error(PAL_X("%sEmpty search paths specified in global.json file: %s"), main_error_prefix, global_json.path.c_str());
        }
    }
    ...
    if (!sdk_exists)
    {
        trace::error(
            PAL_X("\n")
            PAL_X("Download a .NET SDK:\n")
            DOTNET_CORE_DOWNLOAD_URL);
    }

    trace::error(
        PAL_X("\n")
        PAL_X("Learn about SDK resolution:\n")
...
sdk_resolver sdk_resolver::from_nearest_global_file(bool allow_prerelease)
{
    ...
    if (!pal::getcwd(&cwd))
    {
        trace::verbose(PAL_X("Failed to obtain current working dir"));
        assert(cwd.empty());
    }
    else
    {
        trace::verbose(PAL_X("--- Resolving .NET SDK with working dir [%s]"), cwd.c_str());
    }
...
sdk_resolver sdk_resolver::from_nearest_global_file(const pal::string_t& cwd, bool allow_prerelease)
{
    ...
    else
    {
        ...
        if (!global_file.is_data_used())
        {
            ...
            trace::verbose(PAL_X("Invalid global.json [%s]:\n  %s"), global_file.path.c_str(), global_file.error_message.c_str());
            trace::warning(
                PAL_X("Ignoring SDK settings in global.json: the latest installed .NET SDK (%s prereleases) will be used"),
                resolver.allow_prerelease ? PAL_X("including") : PAL_X("excluding"));
        }
...
pal::string_t sdk_resolver::find_nearest_global_file(const pal::string_t& cwd)
{
    if (!cwd.empty())
    {
        ...
        {
            auto file = cur_dir;
            append_path(&file, PAL_X("global.json"));

            trace::verbose(PAL_X("Probing path [%s] for global.json"), file.c_str());
            if (pal::file_exists(file))
            {
                trace::verbose(PAL_X("Found global.json [%s]"), file.c_str());
                return file;
            }
            ...
            {
                trace::verbose(PAL_X("Terminating global.json search at [%s]"), parent_dir.c_str());
                break;
            }
        }
    }
...
sdk_resolver::global_file_info sdk_resolver::parse_global_file(const pal::string_t& global_file_path)
{
    ...
    trace::verbose(PAL_X("--- Resolving SDK information from global.json [%s]"), global_file_path.c_str());
    ...
    const auto& sdk = json.document().FindMember(PAL_X("sdk"));
    if (sdk == json.document().MemberEnd() || sdk->value.IsNull())
    {
        // Missing SDK is treated as success (use default resolver)
        trace::verbose(PAL_X("Value 'sdk' is missing or null in [%s]"), global_file_path.c_str());
    ...
    {
        ret.error_message = PAL_X("Expected a JSON object for the 'sdk' value");
        return ret;
    }

    const auto& version_value = sdk->value.FindMember(PAL_X("version"));
    if (version_value == sdk->value.MemberEnd() || version_value->value.IsNull())
    {
        trace::verbose(PAL_X("Value 'sdk/version' is missing or null in [%s]"), global_file_path.c_str());
    }
    else
    {
        if (!version_value->value.IsString())
        {
            ret.error_message = PAL_X("Expected a string for the 'sdk/version' value");
            return ret;
        }

        if (!fx_ver_t::parse(version_value->value.GetString(), &requested_version))
        {
            ret.error_message = utils::format_string(PAL_X("Version '%s' is not valid for the 'sdk/version' value"), version_value->value.GetString());
            return ret;
        }
    ...
    const auto& roll_forward_value = sdk->value.FindMember(PAL_X("rollForward"));
    if (roll_forward_value == sdk->value.MemberEnd() || roll_forward_value->value.IsNull())
    {
        trace::verbose(PAL_X("Value 'sdk/rollForward' is missing or null in [%s]"), global_file_path.c_str());
    }
    else
    {
        if (!roll_forward_value->value.IsString())
        {
            ret.error_message = PAL_X("Expected a string for the 'sdk/rollForward' value");
            return ret;
        }
        ...
        {
            ret.error_message = utils::format_string(PAL_X("The roll-forward policy '%s' is not supported for the 'sdk/rollForward' value"), roll_forward_value->value.GetString());
            return ret;
        }
        ...
        {
            ret.error_message = utils::format_string(PAL_X("The roll-forward policy '%s' requires a 'sdk/version' value"), roll_forward_value->value.GetString());
            return ret;
        }
    }

    const auto& allow_prerelease_value = sdk->value.FindMember(PAL_X("allowPrerelease"));
    if (allow_prerelease_value == sdk->value.MemberEnd() || allow_prerelease_value->value.IsNull())
    {
        trace::verbose(PAL_X("Value 'sdk/allowPrerelease' is missing or null in [%s]"), global_file_path.c_str());
    }
    else
    {
        if (!allow_prerelease_value->value.IsBool())
        {
            ret.error_message = PAL_X("Expected a boolean for the 'sdk/allowPrerelease' value");
            return ret;
        }
        ...
        {
            trace::warning(PAL_X("Ignoring the 'sdk/allowPrerelease' value in [%s] because a prerelease version was specified"), global_file_path.c_str());
            allow_prerelease = true;
        }
    }

    const auto& paths_value = sdk->value.FindMember(PAL_X("paths"));
    if (paths_value != sdk->value.MemberEnd() && !paths_value->value.IsNull())
    {
        if (!paths_value->value.IsArray())
        {
            ret.error_message = PAL_X("Expected an array for 'sdk/paths' value");
            return ret;
        }
        ...
        for (uint32_t i = 0; i < paths_array.Size(); ++i)
        {
            ...
            if (!path.IsString())
            {
                trace::warning(PAL_X("Ignoring non-string 'sdk/paths[%u]' value in [%s]"), i, global_file_path.c_str());
                continue;
            }
    ...
    const auto& error_message_value = sdk->value.FindMember(PAL_X("errorMessage"));
    if (error_message_value != sdk->value.MemberEnd() && !error_message_value->value.IsNull())
    {
        if (!error_message_value->value.IsString())
        {
            ret.error_message = PAL_X("Expected a string for the 'sdk/errorMessage' value");
            return ret;
        }
    ...
    {
        ret.error_message = utils::format_string(PAL_X("Version '%s' is not valid for the 'sdk/version' value. SDK feature bands start at 1 - for example, %d.%d.100"), requested_version.as_str().c_str(), requested_version.get_major(), requested_version.get_minor());
...
{
    trace::verbose(PAL_X("Searching for SDK versions in [%s]"), dir.c_str());
    ...
    if (exact_match_preferred() && !requested_version.is_empty())
    {
        ...
        {
            trace::verbose(PAL_X("Found requested SDK [%s]"), probe_path.c_str());
        ...
        {
            trace::verbose(PAL_X("Ignoring version [%s] without ") SDK_DOTNET_DLL, requested_version.as_str().c_str());
        }
    }
    ...
    sdk_info::enumerate_sdk_paths(
        ...
        [&](const fx_ver_t& version, const pal::string_t& version_str)
        {
            ...
            {
                trace::verbose(PAL_X("Ignoring version [%s] because it does not match the roll-forward policy"), version_str.c_str());
                return true;
            }
            ...
            {
                trace::verbose(
                    PAL_X("Ignoring version [%s] because it is not a better match than [%s]"),
                    version_str.c_str(),
                    resolved_version_str.empty() ? PAL_X("none") : resolved_version_str.c_str()
                );
                return true;
            }
        ...
        {
            trace::verbose(
                PAL_X("Version [%s] is a better match than [%s]"),
                version_str.c_str(),
                resolved_version_str.empty() ? PAL_X("none") : resolved_version_str.c_str()
            );
