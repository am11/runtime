...
{
    trace::verbose(PAL_X("--- Begin breadcrumb write"));

    auto instance = std::make_shared<breadcrumb_writer_t>(files);
    if (instance->m_breadcrumb_store.empty())
    {
        trace::verbose(PAL_X("Breadcrumb store was not obtained... skipping write."));
        return nullptr;
    }
    ...
    trace::verbose(PAL_X("Breadcrumbs will be written using a background thread"));
...
void breadcrumb_writer_t::write_callback()
{
    ...
    for (const auto& file : m_files)
    {
        ...
        pal::string_t file_name = PAL_X("netcore,") + file;
    ...
    trace::verbose(PAL_X("--- End breadcrumb write %d"), successful);
...
void breadcrumb_writer_t::write_worker_callback(breadcrumb_writer_t* p_this)
{
    ...
    try
    {
        trace::verbose(PAL_X("Breadcrumb thread write callback..."));
        p_this->write_callback();
    }
    catch (...)
    {
        trace::warning(PAL_X("An unexpected exception was thrown while leaving breadcrumbs"));
    }
}
...
void breadcrumb_writer_t::end_write()
{
    ...
    {
        trace::verbose(PAL_X("Waiting for breadcrumb thread to exit..."));

        // Block on the thread to exit.
        m_thread.join();
    }
    trace::verbose(PAL_X("Done waiting for breadcrumb thread to exit..."));
}
